...
void CPUMetrics::updateCPU()
{
	...
	if (fp)
	{
		...
		while(fgets(lineBuffer, 1024, fp) && (lineBuffer[0] == 'c'))
		{
			...
			if((cpuIndex > -1) && (cpuIndex < sizeof(m_CPUUsage)))
			{
				...
				{
					delta[lpsvindex] =
							(float)processorTicks[lpsvindex] - (float)procStatTicks[cpuIndex * num_lpsvs + lpsvindex];
					totalUsage += delta[lpsvindex];

					// Now store the ticks for later
					procStatTicks[cpuIndex * num_lpsvs + lpsvindex] = processorTicks[lpsvindex];
				}
...
#if DUMP_CPU_DATA
	for(unsigned int i = 0; i < m_nProcessors; i++)
	{
